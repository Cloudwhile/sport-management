...
const routes: RouteRecordRaw[] = [
  ...
  },
  {
    path: '/class',
    name: 'ClassLayout',
    component: () => import('@/views/class/Layout.vue'),
    meta: { requiresAuth: true, role: 'class' },
    children: [
      {
        path: '',
        redirect: '/class/data-entry',
      },
      {
        path: 'data-entry',
        name: 'DataEntry',
        component: () => import('@/views/class/DataEntry.vue'),
        meta: { title: '数据录入' },
      },
      {
        path: 'records',
        name: 'ClassRecords',
        component: () => import('@/views/class/Records.vue'),
        meta: { title: '录入记录' },
      },
    ],
  },
